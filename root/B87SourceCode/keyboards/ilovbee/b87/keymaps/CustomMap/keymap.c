...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_BL] = LAYOUT( /* Base */
        ...
        KC_LCTL,  KC_LGUI,  KC_LALT,                                  KC_SPC,                                 KC_RALT,  QK_LLCK,  MO(1),   KC_RCTL,  KC_LEFT,   KC_DOWN,  KC_RGHT),
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    // Base Layer (Layer 0)
    if (layer == 0) {
        // Set the base color to light blue
        rgb_matrix_set_color_all(0, 0, 255);  // Light blue
    }
    
    // Layer 1 (Reactive highlighting only for keys with bindings)
    if (layer == 1) {
        for (uint8_t row = 0; row < MATRIX_ROWS; ++row) {
            for (uint8_t col = 0; col < MATRIX_COLS; ++col) {
                ...
                // Only highlight keys that are not KC_TRNS (i.e., they have a binding)
                if (keycode > KC_TRNS) {
                    // Set these keys to a different color (e.g., yellow) to indicate bindings
                    rgb_matrix_set_color(index, RGB_BLUE);
                } else {
                    // For unbound keys (KC_TRNS), set them to light blue (same as base layer)
                    rgb_matrix_set_color(index, 0, 0, 255);  // Light blue
                }
            }
        }
    }
    
    // Apply the reactive color shift for pressed keys using RGB_MATRIX_SOLID_REACTIVE
    rgb_matrix_layer_power();  // Ensure that the reactive effect is running on the layer
